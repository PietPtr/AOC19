...
parse (memory, pc, rb) = case (opcode) of
    1 -> (Add (makeValue 0) (makeValue 1) (makeAddress 2))
    2 -> (Multiply (makeValue 0) (makeValue 1) (makeAddress 2))
    3 -> (Input (makeAddress 0))
    4 -> (Output (makeValue 0))
    5 -> (JumpTrue (makeValue 0) (makeValue 1))
    6 -> (JumpFalse (makeValue 0) (makeValue 1))
    7 -> (IsLess (makeValue 0) (makeValue 1) (makeAddress 2))
    8 -> (Equals (makeValue 0) (makeValue 1) (makeAddress 2))
    9 -> (AdjRelBase (makeValue 0))
    99 -> (Halt)
    ...
    where
        ...
        makeValue index = case (modes !! index) of
            0 -> memory !! (args !! index)
            1 -> args !! index
            2 -> memory !! (rb + (args !! index))

        makeAddress index = case (modes !! index) of
            0 -> args !! index
            1 -> args !! index
            2 -> rb + (args !! index)
...
calcrb :: Instruction -> (Int -> Int)
calcrb (AdjRelBase rbOffset) = (rbOffset +)
calcrb _ = (0 +)

runner :: [Int] -> State -> [Int] -> [Int]
runner input (memory, pc, rb) output = result
    where
        instruction = parse (memory, pc, rb)
        result = if (instruction == (Halt)) then (output) else
            runner input' (memory', pc', rb') output'
        ...
        rb' = (calcrb instruction) rb

run source input = runner input (source ++ [0,0..], 0, 0) []
...
test 7 = [109,1,204,-1,1001,100,1,100,1008,100,16,101,1006,101,0,99]
test 8 = [1102,34915192,34915192,7,4,7,99,0]
test 9 = [104,1125899906842624,99]
