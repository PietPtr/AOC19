...
occupies' :: Part -> Coord -> [(Coord, Int)] -> Int -> [(Coord, Int)]
occupies' (U 0) _ coords _ = coords
occupies' (D 0) _ coords _ = coords
occupies' (L 0) _ coords _ = coords
occupies' (R 0) _ coords _ = coords
occupies' (U a) (x, y) coords steps = ((x, y-1), steps) : (occupies' (U (a-1)) (x, y-1) coords (steps + 1))
occupies' (D a) (x, y) coords steps = ((x, y+1), steps) : (occupies' (D (a-1)) (x, y+1) coords (steps + 1))
occupies' (L a) (x, y) coords steps = ((x-1, y), steps) : (occupies' (L (a-1)) (x-1, y) coords (steps + 1))
occupies' (R a) (x, y) coords steps = ((x+1, y), steps) : (occupies' (R (a-1)) (x+1, y) coords (steps + 1))

occupies :: Part -> Coord -> Int -> [(Coord, Int)]
occupies part from steps = occupies' part from [] steps
...
occupances :: Path -> Coord -> Int -> [(Coord, Int)]
occupances [] _ _ = []
occupances (part:path) from steps = (occupies part from steps) ++ (occupances path (to part from) (steps + nextSteps part))

-- intersect' :: (Eq a) => [(a, b)] -> [(a, b)] -> [(a, b)] -> [(a, b)]
intersect' [] ys result = result
intersect' (x:xs) ys result
    | value >= 0 = intersect' xs ys ((x, ys !! value):result)
    | otherwise = intersect' xs ys result
    where
        value = fromMaybe (-1) $ (fst x) `elemIndex` (map fst ys)

intersect xs ys = intersect' xs ys []

crossings = intersect (occupances (wire 1) (0, 0) 0) (occupances (wire 2) (0, 0) 0)

dist :: (Int, Int) -> Int
dist (x, y) = (abs x) + (abs y)

-- answer = sort $ map dist crossings
answer = (sort $ map (\x -> (snd (fst x)) + (snd (snd x))) crossings) !! 0 + 2
