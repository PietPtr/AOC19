...
step memory pc input output
    | ins == 1 = step (operate (+) memory pc) (pc+4) input output
    | ins == 2 = step (operate (*) memory pc) (pc+4) input output
    | ins == 3 = step (save input memory pc) (pc+2)
    | ins == 99 = output
    where
        ins = memory !! pc

input = 1:[0,0..]
start = step source 0 input []
